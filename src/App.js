...
function App() {
  ...
  const formik = useFormik({
    ...
    validate: values => {
      let errors = {};
      if (!values.email) {
        errors.email = <span style={{ color: 'blue' }}>Required</span>;
      } else if (!/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(values.email)) {
        errors.email = <span style={{ color: 'red' }}>Username should be an email</span>;
      }
      if (!values.password) errors.password = <span style={{ color: 'blue' }}>Required</span>;
      console.log(errors);
      return errors;
    }
  });
      ...
      <div className="login__page">
        <div className="container__login">
          <form onSubmit={formik.handleSubmit}>
            ...
            <button type="submit" id="submitBtn" style={{ backgroundColor: 'lightblue' }}>Login</button>
